...
title: Alignment
description:
  pls uses variable alignment when listing nodes to account for leading dots and to ensure that the
  first characters in node names line up.
---
...
`--align`/`-a` can be used to turn alignment on or off. File names are aligned by default because
