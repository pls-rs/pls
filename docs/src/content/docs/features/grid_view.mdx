---
title: Grid view
description: pls can display node names in two dimensions by showing multiple nodes per row
...
[`--down`/`-D` flag](/features/direction/).

:::caution

Note that the grid view is incompatible with the
[detail view](/features/detail_view/). In case of conflict, the detail view will
...
[symlink tracing](/features/symlinks/) (which can take large amounts of
...
Note that the grid view is also incompatible with
[collapsing](/features/collapse/) (which leads to nodes that expand to multiple
lines). In case of conflict, the grid view will be used and collapsing will be
turned off.
