---
title: Sorting
description: pls offers an unmatched 18 bases for sorting nodes and 2 directions for each
...
values. All values except `none` can optionally be suffixed with an underscore
`_` to reverse their direction.
