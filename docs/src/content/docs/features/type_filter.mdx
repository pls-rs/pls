...
title: Type filter
description:
  pls provides a way to filter the contents by node type which, combined with the name filter, makes
  it easier to find what you are searching for.
---
...
`--typ`/`-t` can be used to select the node types the user wants to see. The flag can be specified
multiple times to enable multiple file types. Each time it can take one of these values.
...
When parsing the `--typ`/`-t` flag, values are read from the CLI, in order, and added to a vector of
node types till we encounter a shorthand value. If the shorthand value is `none`, the vector is
cleared. If the shorthand value is `all`, the vector is filled with all the node types.
