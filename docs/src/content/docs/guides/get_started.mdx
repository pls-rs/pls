---
title: Get started
description: Installing and using pls is easy and straightforward. pls can be installed via
...
You can suffix `@<version>` to the package name to install a specific version.
The current latest version is v<Version />. Or you can suffix `@latest` to get
the latest at the time of installation.

### Build from source

If you have the Rust toolchain and a C compiler installed on your computer, you
can build from source locally for your operating system and architecture (libgit2
is vendored and compiled during the build). Also, we welcome your
...
You can use `--version` to install a specific version. The current latest
...
you can replace steps 1 through 3 below with this one command. If you've
configured Eget to use a target that's already on your `$PATH`, you can also
skip step 4.
