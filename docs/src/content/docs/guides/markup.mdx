...
title: Markup
description:
  pls uses a novel approach towards styling in the terminal using XML-like tags to specify colors
  and formatting.
---

The terminal is a unique interface. It is purely text-based yet capable of displaying colors and
formatting, using ANSI escape codes. These codes are difficult to remember and use, so <Pls /> uses
a custom approach.

## Markup

The markup language used by <Pls /> is similar to XML. The directives are written inside `<` and `>`
and wrapped around the text to style. The closing tag always matches the last opened tag, regardless
of the text inside it, so it's customary to leave it blank.
...
Terminals can style text in many ways. <Pls /> allows you to use any permutation of these styles in
your configs.
...
Not all terminals support every style. For example, iTerm2 does not support `blink`. The only style
guaranteed to work is `hidden` because it uses a custom implementation that drops the hidden text
entirely.
...
Color support in terminals can range from 16 named colors to 16 million RGB colors! <Pls /> allows
you to use all the colors supported by your terminal.
...
Some themes like [Solarized](https://ethanschoonover.com/solarized/) may opt to use the bright color
space for additional colors not covered in the regular set (like orange in `bright_red`, violet in
`bright_magenta`).
...
<Pls /> also supports using RGB colors. These colors can be specified using a triplet of three `u8`
