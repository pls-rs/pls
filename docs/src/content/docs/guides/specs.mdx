---
title: Specs
descriptions: Specs are the backbone of pls, enabling it to match individual nodes to style
...
adjacent node. Collapsing can be performed in three ways.
...
- **sub**
  ...
  In this case, the parent's name is built by expanding a replacement template
  against the capture groups of the spec's own `pattern`. This handles cases
  that `name` and `ext` cannot, such as transformations in the middle of a name.
  For example, a pattern of `^(?<base>.+)\.min\.js$` with a substitution of
  `$base.js` collapses `app.min.js` into `app.js`. Both `$name`/`${name}` and
