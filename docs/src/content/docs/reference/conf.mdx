...
<Pls /> uses a cascading system for multiple config files. You can have config files at the
...
You can also use emojis as the glyphs but it's not recommended as you can run into issues regarding
character width in some terminals.
...
list of node specs, in ascending order of specificity
  ...
  <div slot="subfields">
    ...
    <DocBlock title="icons" type="seq<str>">
    ...
    <DocBlock title="collapse" type="Collapse">
      ...
      <div slot="examples">
        ...
        ```yaml
        collapse:
          sub: $base.js # with pattern `^(?<base>.+)\.min\.js$`, collapses `app.min.js` into `app.js`
        ```
      </div>
    </DocBlock>
  ...
  <div slot="subfields">
    ...
    <DocBlock title="typ" type="map<str(Typ), TypInfo>" subfieldsType="TypInfo">
      ...
      <div slot="subfields">
        ...
        <DocBlock title="icons" fqTitle="typ__icons" type="seq<str>">
          the fallback icons for the node type, used if no other icon is found;
...
You can see the `.pls.yml` files from <Pls />'s source code itself to get a feel for how these
